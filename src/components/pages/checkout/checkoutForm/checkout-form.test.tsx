...
const MockComponent = () => (
    <TestComponentWrapper> <CheckoutForm formProps={mockFormProps} /> </TestComponentWrapper>
)
...
describe('testing checkout ', () => {
    ...
    test('should show the user entered values in the form fields', async () => {
        ...
        test('form should not be submitted if the required fields are empty', async () => {
            ...
            expect(mockFormProps.handleFormSubmit).not.toHaveBeenCalled()
        }),
        ...
        test('loading state shows on the button once the form is being submitted', () => {
            expect(screen.getByTestId('checkout-btn')).toHaveTextContent('Doing Things')
            expect(screen.getByTestId('checkout-btn')).not.toHaveTextContent('Submit')
            expect(screen.getByTestId('checkout-btn')).toHaveClass('opacity-40')
        })
})