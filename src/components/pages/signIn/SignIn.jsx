  ...
  return (
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-y-6 text-[.8rem]">
          <Input label='Email'styles='px-[12px]' type="email" {...register('email',{required:true})} />
        <Input label='Password' styles='px-[12px]' type={view ? 'text' : 'password'} {...register('password', { required: true })} icon={ <ViewPassword  onClick={()=> setView(!view)} view={view}/>} />
            
          <p className="text-[.8rem] text-[var(--red)] h-1 -mt-5 mb-3">
            {errors}
          </p>
            <Button type='submit' styles={`font-medium text-[var(--white)] bg-[var(--black)] w-[65%] rounded-[24px] h-[48px] px-[24px] hover:bg-[var(--pry-col)] transition-all duration-300 ${
              loading ? "opacity-70 " : "opacity-100"
            }`}
            onClick={() => setIsVisible(!isValid)}
          > {loading ? "Doing Things" : "Create Account"}</Button>
      </form>
     <p className="text-[.8rem] mt-4">Don&apos;t have an account? <small className="underline cursor-pointer" onClick={()=> dispatch(showEntry(id))}>Create Account</small></p>
    </section>
    </>
  )
}
