...
describe('Cart Component', () => {
    test('Uses preloaded state to render', () => {
        const preloadedState = {
            cart: {
                products:
                    [createProductWithQuantity(2)]
                ,
            },
        };
        const { getByText, getByTestId, getByRole } = renderWithProviders(<Cart />, { preloadedState })
        // Check that static text content is rendered
        ...
        test('Displays updated content when more products are added to the cart', () => {
            const preloadedState = {
                cart: {
                    products:
                        [createProductWithQuantity(8)]
                    ,
                },
            };
            const { getByRole } = renderWithProviders(<Cart />, { preloadedState })
        ...
        test('Displays updated content when more products are added to the cart', () => {
            const preloadedState = {
                cart: {
                    products:
                        [createProductWithQuantity(10)]
                    ,
                },
            };
            const { getByRole } = renderWithProviders(<Cart />, { preloadedState })
            expect(getByRole('button', { name: /Out of Stock/i })).toBeInTheDocument();
        }),
        test('cart empty when no product has been added', () => {
            ...
            const { getByTestId, getByRole } = renderWithProviders(<Cart />, { preloadedState })
