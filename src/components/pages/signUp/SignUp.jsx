...
import { specialChars,hasNumber } from "../../../utils/testInput/testInput";


function SignUp({ id }) {
  const [birthday, setBirthday] = useState({ month: months, day: days(31) });
  const {month,day} = birthday
  const { register, handleSubmit, isLoading, isSubmitSuccessful, watch, formState } = useForm();
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-y-6 text-[.8rem]">
          ...
          <section className="">
            <Input label='Password' type={!view1 ? 'password' : 'text'} styles='relative px-[12px]' placeholder='Password must be at least 8 characters' {...register('password', { required: true })} icon={ <ViewPassword onClick={()=> setView1(!view1)} view={view1}/>} />
          </section>
          <p className="h-4">{pvalid}</p>
          <Input label='Confirm Password' styles='px-[12px]' type={!view2 ? 'password' : 'text'}  {...register("cpassword", { required: true , validate: (val) => {
         ...
         return "Your passwords do no match";
          ...
          <Button type='submit' onClick={()=> setIsVisible(!isValid)} styles={`font-medium text-[var(--white)] bg-[var(--black)] w-[65%] rounded-[24px] h-[48px] px-[24px] hover:bg-[var(--pry-col)] transition-all duration-300 ${loading ? 'opacity-70 ' : 'opacity-100'}`} disabled={loading} >{loading ? 'Doing Things' : 'Create Account'}</Button>
      </form>
